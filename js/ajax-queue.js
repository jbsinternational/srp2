...
//											AJAX QUEUE
//
// http://cmarshall.com/MySoftware/ajax/ajax_queue.js
//
//***********************************************************************************

var	g_ajax_obj = new c_ajax_object;	// This will be the gobal AJAX object. There 
                                       // can be only one...
...
c_ajax_object.prototype.Enqueue = function ( ) {
	...
	this._dm_queue[this._dm_queue.length] = new Array ( 
      this._dm_pre_queue_in_url, 
      this._dm_pre_queue_in_callback,
...
c_ajax_object.prototype._CallXMLHTTPObject = function ( in_url, in_method ) {
   try {
      ...
      if ( in_method == "POST" ) {
         // 2015.05.28 pjm: uncomment the following code to send
         // the value as content rather than as an appendage to
         // the url.
         // var rmatch = /^(.*)\&value=(.*)$/.exec ( in_url );
         // in_url = rmatch[1];
...
function SimpleAJAXCall ( in_uri, in_callback, in_method, in_param ) {
   ...
   console.log("SimpleAJAXCall: " + in_method + " " + in_uri);
